...
def _render_all_tabs(
    ...
    with tab2:
        ...
        _plot_stress_timeseries(history, upto_idx=upto)
        # Nutrient traffic lights (legacy proxy retained)
        ...
        if n_total >= 120:
            ...
            n_badge = "🔴 N low"
        st.markdown(n_badge)
        # Event-derived P stress is plotted above; badge kept simple
...
def _plot_biomass(history: Mapping[str, Any], *, upto_idx: int | None = None) -> None:
    ...
    if history.get("water_stress"):
        ...
        with c1:
            ws_last = history["water_stress"][-1 if upto_idx is None else upto_idx - 1]
            st.metric("Water stress", f"{float(ws_last):.2f}")
        with c2:
            # Prefer event-derived N stress if present; fall back to proxy
            n_series = history.get("n_stress", [])
            n_display: Optional[float] = None
            idx = -1 if upto_idx is None else upto_idx - 1
            ...
            if n_display is None:
                ...
                n_target = 150.0
                n_display = max(0.0, min(1.0, float(n_total or 0.0) / n_target))
            st.metric("N stress", f"{n_display:.2f}")
        with c3:
            # P stress shown when available
            p_series = history.get("p_stress", [])
            idx = -1 if upto_idx is None else upto_idx - 1
            p_val = None
            if p_series and len(p_series) > 0:
                p_val = p_series[idx]
