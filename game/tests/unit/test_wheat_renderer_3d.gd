...
const Renderer = preload("res://scripts/wheat_renderer_3d.gd")
...
func test_mature_keeps_structure() -> void:
	# Wheat renders a fixed tiller structure; growth scales leaf length and stem
	# height rather than node count, so mature is non-decreasing vs seedling.
