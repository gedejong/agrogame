...
def test_load_sample_params() -> None:
    lib = load_library("data/samples/crops.yaml")
...
def test_other_crops_present() -> None:
    lib = load_library("data/samples/crops.yaml")
    assert {"maize", "wheat", "potato"}.issubset(set(lib.crops.keys()))
